...
class CentrisB(Syringe):
    """
    Class to control Centris -B pumps with distribution valves. Provides front-
    ...
    PORT_DICT = {b'i': 1, b'b': 2, b'o': 3, b'e': 4, b'n': 5, b'1': 1,
                 b'2': 2, b'3': 3, b'4': 4, b'5': 5, b'6': 6}

    SPEED_CODES = {0: 200000, 1: 180000, 2: 160000, 3: 160000, 4: 120000,
                   5: 100000, 6: 90000, 7: 80000, 8: 70000, 9: 60000,
                   10: 50000, 11: 40000, 12: 30000, 13: 20000, 14: 10000,
                   15: 9000, 16: 8000, 17: 7000, 18: 6000, 19: 5000,
                   20: 4000, 21: 3000, 22: 2000, 23: 1000, 24: 900, 25: 800,
                   26: 700, 27: 600, 28: 500, 29: 400, 30: 300, 31: 200,
                   32: 100, 33: 90, 34: 80, 35: 70, 36: 60, 37: 50, 38: 40,
                   39: 30, 40: 20, 41: 10, 42: 9, 43: 8, 44: 7, 45: 6, 46: 5,
                   47: 4, 48: 3, 49: 2, 50: 1}

    def __init__(self, com_link, num_ports=6, syringe_ul=1000, direction='CW',
                 microliter=False, waste_port=6, slope=8, init_force=7,
        ...
        Kwargs:
            ...
            `syringe_ul` (int) : syringe volume in microliters
                [default] - 1000
            `microliter` (bool) : whether or not to operate in microliter mode
                                  opposed to increment mode
                [default] - False (factory default)
            `waste_port` (int) : waste port for `extractToWaste`-like
                                 convenience functions
                [default] - 6 (factory default for init out port)
            `slope` (int) : slope setting
                [default] - 8 (factory default)
            `init_force` (int) : initialization force or speed
                4-25 - speed code X
                [default] - 7 (factory default for init speed)
            `debug` (bool) : turns on debug file, which logs extensive debug
                             output to 'centris_debug.log' at
                             `debug_log_path`
                [default] - False
        ...
        """
        super(CentrisB, self).__init__(com_link)
        self.num_ports = num_ports
        self.syringe_ul = syringe_ul
        self.direction = direction
        self.microliter = microliter
    ...
    def initDebugLogging(self, debug_log_path):
        ...
        self.logger = logging.getLogger('CentrisB')
        fp = debug_log_path.rstrip('/') + '/centris_debug.log'
    ...
    def init(self, init_force=None, direction=None, in_port=None,
        ...
        in_port = in_port if in_port is not None else 1
    ...
    def primePort(self, in_port, volume_ul, speed_code=None, out_port=None,
        ...
        if volume_ul > self.syringe_ul:
            ...
            for x in range(int(num_rounds)):
                self.changePort(out_port, from_port=in_port)
                self.movePlungerAbs(self.syringe_ul)
            ...
            if remainder_ul != 0:
                self.changePort(out_port, from_port=in_port)
                self.movePlungerAbs(remainder_ul)
                self.changePort(in_port, from_port=out_port)
                self.movePlungerAbs(0)
                delay = self.executeChain()
                self.waitReady(delay)
    ...
    def executeChain(self, minimal_reset=False):
        ...
        tic = time.time()
        #print('Sent command:', self.cmd_chain)
    ...
    @execWrap
    def dispense(self, in_port, dist, out_port=None):
        """
        Aspirate from the out_port and dispense to the in_port

        Edited 3/11/2020

        Args:
            'in_port' (int) : port to dispense into
            `dist` (int) : plunger range
                (0-syringe_ul) in microliter mode
                (0-181490) in increment mode
         Kwargs:
            'out_port' (int) : exhaust port (move plunger)
        ...
        self.movePlungerRel(int(-dist))

    @execWrap
    def aspirate(self, in_port, dist, out_port=None):
        """
        Zero the plunger in the out_port and aspirate the in_port

        Edited 3/11/2020

        Args:
            'in_port' (int) : port to aspirate from
            `dist` (int) : plunger range
                (0-syringe_ul) in microliter mode
                (0-181490) in increment mode
        Kwargs:
    ...
    def movePlungerAbs(self, abs_position):
        ...
        if self.sim_state['microliter']:
            if not 0 <= abs_position <= self.syringe_ul:
                raise (ValueError('`abs_position` must be between 0 and syringe_ul'
                                  ' when operating in microliter mode'))
            cmd_string = 'A{0},{1}'.format(abs_position, int(self.sim_state['microliter']))
        else:
            if not 0 <= abs_position <= 181490:
                raise (ValueError('`abs_position` must be between 0 and 181490'
                                  ' when operating in increment mode'))
            cmd_string = 'A{0},{1}'.format(abs_position, int(self.sim_state['microliter']))
    ...
    def movePlungerRel(self, rel_position):
        ...
        error code 3 from the Centris, raising a `SyringeError`

        Args:
            `rel_position` (int) : relative plunger position
                if rel_position < 0 : plunger moves up (relative dispense)
                if rel_position > 0 : plunger moves down (relative pickup)

        """
        self.logCall('movePlungerRel', locals())

        if rel_position < 0:
            cmd_string = 'D{0},{1}'.format(abs(rel_position),int(self.sim_state['microliter']))
        else:
            cmd_string = 'P{0},{1}'.format(rel_position,int(self.sim_state['microliter']))
    ...
    def setSpeed(self, speed_code):
        ...
        if not 0 <= speed_code <= 50:
            raise (ValueError('`speed_code` [{0}] must be between 0 and 50'
                              ''.format(speed_code)))
    ...
    @execWrap
    def setStartSpeed(self, rate):
        """
        Set start speed

        Args:
            `rate` (float) : start speed
                () in microliter/sec
                (800-32000) in increment/sec

        """
        self.logCall('setStartSpeed', locals())

        cmd_string = 'v{0},{1}'.format(rate, int(self.sim_state['microliter']))
        self.sim_speed_change = True
        self.cmd_chain += cmd_string

    @execWrap
    def setTopSpeed(self, rate):
        """
        Set top speed

        Args:
            `rate` (float) : top speed
                () in microliter/sec
                (1-200000) in increment/sec
        """
    ...
    @execWrap
    def setCutoffSpeed(self, rate):
        """
        Set cutoff speed

        Args:
            `rate` (float) : cutoff speed
                () in microliter/sec
                (80-64000) in increment/sec
        """
        self.logCall('setCutoffSpeed', locals())

        cmd_string = 'c{0},{1}'.format(rate, int(self.sim_state['microliter']))
        self.sim_speed_change = True
        self.cmd_chain += cmd_string
    ...
    def setSlope(self, slope_code, chain=False):
        ...
        if not 1 <= slope_code <= 40:
            raise (ValueError('`slope_code` [{0}] must be between 1 and 40'
                              ''.format(slope_code)))
        cmd_string = 'L{0},{1}'.format(slope_code, slope_code)
        self.sim_speed_change = True
        self.cmd_chain += cmd_string

    @execWrap
    def setBacklash(self, increments):
        """ Set backlash compensation increments by `increments` (0-4000) """
        self.logCall('setBacklash', locals())

        if not 0 <= increments <= 4000:
            raise (ValueError('`increments` [{0}] must be between 0 and 4000'
                              ''.format(increments)))
        cmd_string = 'K{0}'.format(increments)
        self.sim_state['backlash'] = increments
        self.cmd_chain += cmd_string
    ...
    # Non-chainable control commands

    def repeatCmd(self):
        """ Repeat the last command or program string """
        self.logCall('repeatCmd', locals())

        cmd_string = 'X'
        self.sendRcv(cmd_string)
    ...
    def getPlungerPos(self):
        ...
        Return:
            'plunger_pos' (float): (0-syringe_ul) in microliter mode
                                 (0-181490) in increment mode
        """
        ...
        self.state['plunger_pos'] = float(data)
        return self.state['plunger_pos']

    def getStartSpeed(self):
        """
        Returns the start speed as an int

        Return:
            'start_speed' (float): (ul/sec) in microliter mode
                                 (increment/sec) in increment mode
        """
        ...
        data = self.sendRcv(cmd_string)
        self.state['start_speed'] = float(data)
        return self.state['start_speed']

    def getTopSpeed(self):
        """
        Returns the top speed as an int

        Return:
            'top_speed' (float): (ul/sec) in microliter mode
                               (increment/sec) in increment mode
        """
        ...
        if self.sim_state['microliter']:
            ...
            cmd_string = '?7'
        data = self.sendRcv(cmd_string)
        self.state['top_speed'] = float(data)
        return self.state['top_speed']

    def getCutoffSpeed(self):
        """
        Returns the cutoff speed as an int

        Return:
            'cutoff_speed' (float): (ul/sec) in microliter mode
                                  (increment/sec) in increment mode
        """
        ...
        if self.sim_state['microliter']:
            ...
            cmd_string = '?8'
        data = self.sendRcv(cmd_string)
        self.state['cutoff_speed'] = float(data)
        return self.state['cutoff_speed']

    def getEncoderPos(self):
        """ Returns the current encoder count on the plunger axis (mm) """
    ...
    def getCurPort(self):
        ...
        with self._syringeErrorHandler():
            try:
                port = self.__class__.PORT_DICT[data]
            except ValueError:
    ...
    def getCurTemp(self):
        """ Returns the real-time pump temperature (degF) """
        self.logCall('getCurTemp', locals())

        cmd_string = '?53'
        data = self.sendRcv(cmd_string)
        self.state['temp'] = float(data)
        return self.state['temp']

    def getMaxTemp(self):
        """ Returns the highest pump temperature (degF) """
        self.logCall('getMaxTemp', locals())
    ...
    def getMinTemp(self):
        """ Returns the lowest pump temperature (degF) """
        self.logCall('getMinTemp', locals())
    ...
    def getConfig(self):
        """ Returns the pump configuration (ASCII text) """
        self.logCall('getConfig', locals())
    ...
    def setPump(self, valve_code):
        """ Sets pump/valve configuration info to non-volatile memory """
        self.logCall('setPump', locals())

        if not 0 <= valve_code <= 98:
            raise (ValueError('`valve_code` [{0}] must be between 0 and 98'
                              ''.format(valve_code)))

        cmd_string = 'U{0}'.format(int(valve_code))
        self.sendRcv(cmd_string, execute=True)
    ...
    def sendRcv(self, cmd_string, execute=False):
        ...
        Args:
            `cmd_string` (bytestring) : a valid Tecan Centris command string
        Kwargs:
        ...
        with self._syringeErrorHandler():
            parsed_response = super(CentrisB, self)._sendRcv(cmd_string)
    ...
    def _calcPlungerMoveTime(self, move_steps):
        ...
        Assumes sim_state is in increment units and backlash compensation
            is set to zero

        """
        sd = self.sim_state
        microliter = sd['microliter']

        if microliter:
            # Convert from microliters to increments
            syringe_ul = self.syringe_ul
            start_speed = sd['start_speed'] * syringe_ul / 181490
            top_speed = sd['top_speed'] * syringe_ul / 181490
            cutoff_speed = sd['cutoff_speed'] * syringe_ul / 181490
            slope = sd['slope']
        else:
...
class SmartValveB(Syringe):
    """
    Class to control Smart Valve -B distribution valves. Provides front-
    end validation and convenience functions for valve use.
    """

    DIR_DICT = {'CW': ('I', 'Z', 0), 'CCW': ('O', 'Y', 1)}
    PORT_DICT = {b'i': 1, b'b': 2, b'o': 3, b'e': 4, b'n': 5, b'1': 1,
                 b'2': 2, b'3': 3, b'4': 4, b'5': 5, b'6': 6, b'7': 7,
                 b'8': 8, b'9': 9, b'10': 10, b'11': 11, b'12': 12}

    def __init__(self, com_link, num_ports=12, direction='CW', init_force=7,
                 debug=False, debug_log_path='.'):
        """
        Object initialization function.
        ...
        Kwargs:
            ...
            `init_force` (int) : initialization force or speed
                ...
                [default] - 7 (factory default for init speed)
            `debug` (bool) : turns on debug file, which logs extensive debug
                             output to 'smart_valve_debug.log' at
                             `debug_log_path`
                [default] - False
            `debug_log_path` : path to debug log file - only relevant if
                               `debug` == True.
        ...
        """
        super(SmartValveB, self).__init__(com_link)
        self.num_ports = num_ports
        self.direction = direction
        self.init_force = init_force
        self.config = None
    ...
    def initDebugLogging(self, debug_log_path):
        ...
        self.logger = logging.getLogger('SmartValveB')
        fp = debug_log_path.rstrip('/') + '/smart_valve_debug.log'
        hdlr = logging.FileHandler(fp)
        formatter = logging.Formatter('%(asctime)s %(levelname)s %(message)s')
        hdlr.setFormatter(formatter)
        self.logger.addHandler(hdlr)
    ...
    def init(self, position=None, direction=None):
        """
        Initialize pump. Kwarg 'position' sets valve position after
        initialization. Uses instance `self.direction` if not provided.
        ...
        direction = direction if direction is not None else self.direction
        position = int(position) if position is not None else 1

        cmd_string = 'w{0},{1}'.format(position,
                                       self.__class__.DIR_DICT[direction][2])
        self.sendRcv(cmd_string, execute=True)
        self.waitReady(delay=3)
        return 0  # 0 seconds left to wait
    ...
    def getCurSpeed(self):
        """ Returns the current valve speed in half-steps per second """
        self.logCall('getCurSpeed', locals())

        cmd_string = '?1'
        data = self.sendRcv(cmd_string)
        self.state['speed'] = float(data)
        return self.state['speed']

    def getCurVoltage(self):
        """ Returns the current power supply's voltage """
        self.logCall('getVoltage', locals())

        cmd_string = '?26'
        data = self.sendRcv(cmd_string)
        self.state['voltage'] = float(data)
    ...
    def sendRcv(self, cmd_string, execute=False):
        ...
        self.logDebug('sendRcv: sending cmd_string: {}'.format(cmd_string))
        with self._syringeErrorHandler():
            parsed_response = super(SmartValveB, self)._sendRcv(cmd_string)
            self.logDebug('sendRcv: received response: {}'.format(
                          parsed_response))
            data = parsed_response[0]
            return data
